...
export interface InitOptions {
  ...
  enableXhr?: boolean; // 是否监控 xhr 请求
  enableFetch?: boolean; // 是否监控 fetch 请求
  enableClick?: boolean; // 是否监控 click 事件
  enableError?: boolean; // 是否监听 error 事件
  enableUnhandledrejection?: boolean; // 是否监控 unhandledrejection
  enableHashchange?: boolean; // 是否监听路由 hash 模式变化
  enableHistory?: boolean; // 是否监听路由 history模式变化
  enablePerformance?: boolean; // 是否获取页面性能指标
  enableRecordScreen?: boolean; // 是否开启录屏
  recordScreenTime?: number; // 单次录屏时长
  recordScreenTypeList?: string[]; // 上报录屏的错误列表
  enableWhiteScreen?: boolean; // 是否开启白屏检测
