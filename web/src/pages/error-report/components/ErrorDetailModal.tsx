...
export default function ErrorDetailModal({ visible, record, onClose }: ErrorDetailModalProps) {
  ...
  return (
    <Modal title="错误详情" open={visible} onCancel={onClose} footer={null} width={900} style={{ top: 20 }}>
      <div className="space-y-4 max-h-[70vh] overflow-auto">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <strong>事件类型：</strong>
            {recordAny.event_type || '未知'}
          </div>
          <div>
            <strong>错误子类型：</strong>
            {recordAny.sub_type || '未分类'}
          </div>
          <div>
            <strong>应用ID：</strong>
            {recordAny.app_id || '未知'}
        ...
        {recordAny.device_info && (
          <div>
            <strong>设备信息：</strong>
            <pre className="bg-gray-100 p-3 rounded text-sm mt-2">{JSON.stringify(recordAny.device_info, null, 2)}</pre>
          </div>
        )}
        ...
        {recordAny.os_info && (
          <div>
            ...
            <pre className="bg-gray-100 p-3 rounded text-sm mt-2">{JSON.stringify(recordAny.os_info, null, 2)}</pre>
          </div>
        )}
        ...
        {recordAny.geo_info && (
          <div>
            ...
            <pre className="bg-gray-100 p-3 rounded text-sm mt-2">{JSON.stringify(recordAny.geo_info, null, 2)}</pre>
          </div>
        )}
